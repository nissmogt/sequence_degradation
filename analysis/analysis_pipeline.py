...
def pipeline_replicates(_dca_dir, _sysid, _ncols, thresholds_list, distance_cutoff, npairs=0, NR=0,
    ...
    if n_replicates != NR:
        print(f"NR:{NR}, n_replicates:{n_replicates} don't match. Was this on purpose?")
    avg_neff = neffective_array.mean(axis=0)
    # CONTROL PPV OUTFILE NAME DEPENDING ON ZFILTER VALUE
    pdbid = _sysid
    dir_ppv = os.path.join(dir_results)
    if not os.path.exists(dir_ppv):
        os.makedirs(dir_ppv)
    if zfilter:
        # DEBUG: potential bug with 100 being hardcoded
        output_ppv = os.path.join(dir_ppv, f"{distance_cutoff}A_{pdbid}_ppv_zscore_{dca_score}_{NR}reps.npy")
        output_pair_rank = os.path.join(dir_ppv, f"{distance_cutoff}A_{pdbid}_ppv_rank_z_{dca_score}_{NR}reps.npy")
        output_z = os.path.join(dir_ppv, f"{distance_cutoff}A_avg_z_{pdbid}_{NR}reps.npy")
        output_dist = os.path.join(dir_ppv, f"{distance_cutoff}A_avg_dist_{pdbid}_{NR}reps.npy")
    else:
        if npairs > 0:
            output_ppv = os.path.join(dir_ppv, f"{distance_cutoff}A_{pdbid}_ppv_top{npairs}_{dca_score}_{NR}reps.npy")
        else:
            output_ppv = os.path.join(dir_ppv, f"{distance_cutoff}A_{pdbid}_ppv_top{thresholds_list[-1]}_{dca_score}_{NR}reps.npy")
            output_pair_rank = os.path.join(dir_ppv,
                                            f"{distance_cutoff}A_{pdbid}_ppv_pairrank_top{thresholds_list[-1]}_{dca_score}_{NR}reps.npy")
    ...
    if passthrough:
        ...
        for rep_id in range(NR):
            ...
            for model_id in range(len(model_lengths)):
                ...
                print(f"d:{distance_cutoff} rep: {rep_id} model: {n_degraded_seqs} neff: {neff}  L: {_ncols} map:{map_idx}")
                if plots:
